...
# converting the date to a better format
datetime <- paste(as.Date(df_req$Date), df_req$Time)

df_req$Datetime <- as.POSIXct(datetime)

#This generates plot 2
plot(df_req$Global_active_power~df_req$Datetime, type="l", ylab="Global Active Power (kilowatts)", xlab="")
